...
def Krige_Interpolate(dataframe, sample_name, column_name, SIMS_session=None, label=None):
    ...
    weight=False, nlags=6, 
    #variogram_model="linear",)
    #variogram_model="linear", variogram_parameters={'slope': 5e-18, 'nugget': 6e-7})
    variogram_model="gaussian",variogram_parameters={'sill': 1e-4, 'range': 9e12, 'nugget': 0})
...
ax.fill_between( X_pred, y_pred - 2 * y_std, y_pred + 2 * y_std, alpha=0.3,label="Confidence interval",)

Sample_plot(Samples_db, "Herasil", "17_30si_ratio",ax=ax, SIMS_session=1, label='Session 1')
